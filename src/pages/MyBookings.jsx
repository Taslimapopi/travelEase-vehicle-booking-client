...
const MyBookings = () => {
  ...
  useEffect(() => {
    // fetch(`https://travel-ease-server-kappa.vercel.app/my-bookings?email=${user.email}`,{
    //   headers:{
    ...
    axiosSecure(`/my-bookings?email=${user.email}`).then((data) => {
  ...
  const handleRemoveBookings = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://travel-ease-server-kappa.vercel.app/bookings/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
